...
5. **expo-modules-autolinking** reads `expo-module.config.json` from each package and generates a platform file (Swift/Kotlin) listing all Module classes to instantiate
...
```
expo-module.config.json (per package) expo-modules-autolinking (forked)
  { "windows": {                  →     Reads all packages' expo-module.config.json
...
1. Finds packages with `expo-module.config.json` containing `windows` config
...
1. Each package declares its modules in `expo-module.config.json` under `"windows"` key
...
│   │   ├── ExampleModule.cs
│   │   └── expo-module.config.json # Declares modules for autolinking
│   └── windows/
│       ├── ExpoModulesWindowsCoreExample.sln
